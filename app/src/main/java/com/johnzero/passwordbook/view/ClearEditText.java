...
public class ClearEditText extends AppCompatEditText implements TextWatcher, View.OnFocusChangeListener {
    ...
    private void init(){
        ...
        if(mClearDrawable==null) mClearDrawable=getCompoundDrawablesRelative()[2];
        //根据实际情况设置图标的位置以及大小,getIntrinsicwidth()获取显示出来的大小而不是原图片的大小
        if(mClearDrawable!=null)mClearDrawable.setBounds(-40,0, (int) (mClearDrawable.getIntrinsicWidth()*0.3), (int) (mClearDrawable.getIntrinsicHeight()*0.5));
        //默认设置隐藏图标
    ...
    //到EditText的右边缘之间就算点击了图标，竖直方向就以 EditText高度为边界
    @Override
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        ...
        if(mOnFocusChangeListener!=null) mOnFocusChangeListener.onFocusChange(v,hasFocus);
    }
    ...
    //1、是自己本身的话则调用父类的实现﹔2、是外部设置的就自己处理回调回去
    @Override
    public void setOnFocusChangeListener(OnFocusChangeListener l){
        if(l instanceof ClearEditText) super.setOnFocusChangeListener(l);
        else mOnFocusChangeListener=l;
    }

    @Override
    public void addTextChangedListener(TextWatcher watcher){
        if(watcher instanceof ClearEditText) super.addTextChangedListener(watcher);
        else mTextWatcher=watcher;
    }
}
